...
public struct CompletionKind {
  internal enum Kind {
    ...
    case custom(String, (@Sendable ([String], Int, String) -> [String])?)
    case customAsync(
      String,
      (@Sendable ([String], Int, String) async -> [String])?
    )
  ...
  /// given string run as a shell command when a user requests completion
  /// candidates.
  ///
  ...
  /// The given shell script is included in a completion script when it is
  /// generated.
  ///
  /// The completion script runs when a user requests completion candidates.
  ///
  /// The given shell script must call shell-specific completion functions to
  /// generate completion candidates. Unlike `shellCommand(String)`, `stdout`
  /// output is not used to supply completion candidates. `shellCommand(String)`
  /// is thus simpler to use, but not as flexible as this completion kind.
  ///
  /// If completion candidates generated by Swift must be processed by a custom
  /// shell script, instead use `custom(String, @Sendable @escaping ([String],
  /// Int, String) -> [String])` or its `async` variant.
  ///
  /// Swift Argument Parser does not perform any escaping or quoting on the
  /// given shell script.
  @preconcurrency
  public static func custom(shellScript: String) -> CompletionKind {
    CompletionKind(kind: .custom(shellScript, nil))
  }

  /// If the given shell script is empty, the completion candidates are the
  /// strings in the array returned by the given closure when it is executed in
  /// response to a user's request for completion candidates.
  ///
  /// If the given shell script is not empty, the completion candidates are
  /// calculated by the given shell script. The strings in the array returned
  /// by the given closure when it is executed in response to a user's request
  /// for completion candidates as made available to the given shell script via
  /// the `$completions` shell variable.
  ///
  /// Swift Argument Parser does not perform any escaping or quoting on the
  /// given shell script.
  ///
  ...
  /// generated.
  ///
  /// Completion candidates returned by the given closure are interpreted by the
  /// requesting shell as literals. They must be neither escaped nor quoted;
  /// Swift Argument Parser escapes or quotes them as necessary for the
  /// requesting shell.
  ///
  ...
  /// completion candidates are being requested within the given `[String]`.
  ///
  /// The third argument (a `String`) is the prefix of the word for which
  /// completion candidates are being requested that precedes the cursor.
  ///
  /// ### bash
  ///
  ...
  public static func custom(
    ...
    CompletionKind(kind: .custom(shellScript, completion))
  }

  /// Generates completion candidates using the given async closure.
  ///
  /// The same as `custom(String, @Sendable @escaping ([String], Int, String) ->
  /// [String])`, except that the closure parameter is asynchronous.
  @available(macOS 10.15, macCatalyst 13, iOS 13, tvOS 13, watchOS 6, *)
  public static func custom(
    shellScript: String = "",
    _ completion: @Sendable @escaping ([String], Int, String) async -> [String]
  ) -> CompletionKind {
    CompletionKind(kind: .customAsync(shellScript, completion))
  }

  /// Deprecated; only kept for backwards compatibility.
  ///
  /// The same as `custom(String, @Sendable @escaping ([String], Int, String) -> [String])`,
  ...
  @available(
    *,
    deprecated,
    message:
      "Provide a three-parameter closure instead. See custom(String, @Sendable @escaping ([String], Int, String) -> [String])."
  )
  public static func custom(
